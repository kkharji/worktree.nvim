...
local M = { assert = {}, get = {}, set = {}, perform = {} }
local assert, get, set, perform = M.assert, M.get, M.set, M.perform
...
get.name = function(cwd)
  return Job { "git", "rev-parse", "--abbrev-ref", "HEAD", cwd = cwd, on_exit = msgs.get_name(cwd) }
end
...
get.description = function(branch_name, cwd)
  ...
  args.on_exit = msgs.get_description(cwd)
  return Job(args)
end
...
perform.merge_remote = function(branch_name, cwd)
  ...
  args.on_exit = msgs.merge_remote(remote_name, branch_name)
  return Job(args)
end
...
---@param cwd string
---@return Job
perform.squash_and_merge = function(branch_name, cwd)
  local args = { "git", "merge", "--squash", branch_name, cwd = cwd }
  args.on_exit = msgs.squash_and_merge(branch_name)
  return Job(args)
end
...
perform.push = function(branch_name, cwd)
  ...
  local args = { "git", "push", "-u", remote, branch_name, cwd = cwd, on_exit = msgs.push(branch_name) }
  return Job(args)
end
...
perform.checkout = function(branch_name, cwd)
  ...
  args.on_exit = msgs.checkout(branch_name)
  return Job(args)
end
...
set.name = function(branch_name, new, cwd)
  ...
  args.on_exit = msgs.set_name(branch_name, new)
...
set.description = function(branch_name, description, cwd)
  local path = string.format("branch.%s.description", branch_name)
  return Job { "git", "config", path, description, cwd = cwd, on_exit = msgs.set_description(description) }
end
...
---@return Job
perform.commit = function(heading, body, cwd)
  local args = { "git", "commit", "-m", heading, cwd = cwd }
  body = vim.split(body, "\n")
...
---@param cwd string
---@return Job
perform.fork = function(cwd)
  return Job { "gh", "repo", "fork", "--remote=true", cwd = cwd, on_exit = msgs.fork(cwd) }
end
...
---@param cwd string
---@return Job
set.pr_info = function(fields, cwd)
  local start = assert.is_online(true)
  local cb = fields.cb
  fields.cb = nil
  local args = { "gh", "pr", "edit", on_exit = msgs.pr_update(cwd) }
